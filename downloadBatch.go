...
func init() { //用于判断文件名的后缀
	...
	fileTypeMap.Store("ffd8ffdb004300060405", "jpg")  //JPEG (jpg)
	...
	fileTypeMap.Store("524946468c0d00005745", "webp")  //压缩格式图片(webp)
	fileTypeMap.Store("52494646", "webp")  //压缩格式图片(webp)
...
func (controller *DownloadThreadController) DownloadFile(paramsInfo interface{}) {
	...
	case DownloadFileParams:
		...
		if err != nil {
			...
			if err != nil {
				fmt.Println("Failed to open file1 " + tempFilename)
...
const (
	//folder string = "D:\\home\\download"
	folder string = "/www/wwwroot/file2.kaopuke.com/files_image"
	thread int = 8
)
...
func main() {
	now := time.Now()
	var url string
	//flag.StringVar(&url, "url", "https://dldir1.qq.com/weixin/Windows/WeChatSetup.exe", "The http file's download url")
	//flag.StringVar(&url, "url", "https://down10.zol.com.cn/WebsitePromotion/inst_buychannel_28.exe", "The http file's download url")
	//flag.StringVar(&url, "url", "https://studygolang.com/dl/golang/go1.13.4.linux-amd64.tar.gz", "The http file's download url")
	//flag.StringVar(&url, "url", "https://mmbiz.qpic.cn/mmbiz/sXiaukvjR0RAAk3QV4yokge3NC4uLreJLHjq1tVCFBIdFhibJNvmRtGqZibtVssH52z8VsFB1S4oGMM8mPS31wAmg/640?wx_fmt=jpeg&wxfrom=5&wx_lazy=1&wx_co=1", "The http file's download url")
	//flag.StringVar(&url, "url", "https://upload-images.jianshu.io/upload_images/19826068-8935581745b7451c.png?imageMogr2/auto-orient/strip|imageView2/2/w/352/format/webp", "The http file's download url")
	//flag.StringVar(&folder, "folder", "D:\\home\\download", "The downloaded file save path")
	//flag.IntVar(&thread, "thread", 32, "Tht number of child threads to download file")
	//flag.Parse()
	//for i:=1;i<50;i++ {
	//	url = "https://img-va.myshopline.com/image/store/3400003311/1627028228094/918f4f9dfea649beaed889e6cd585304.webp?w=640&h=853"
		url = "https://imgconvert.csdnimg.cn/aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X2dpZi84alBUMkx3S2swdmp5QUswM0NNMXRrbjQxY3Q0d2tEcUxvaGtCeEF0bFRCcjRLUnZIVjA2SlF5R3lkSVFQbUQ1UkVIVkxVc1hBeG9MYjhQM1JjSHVGdy82NDA?x-oss-process=image/format,png"
		//获取文件名称带后缀
		fileNameWithSuffix:=library.GetFilenameFromUrl(url)
		...
		//获取文件名称(不带后缀)
		if fileType!="" {
			fileNameWithSuffix = strings.TrimSuffix(fileNameWithSuffix, fileType)
		}
		//if strings.Contains(fileNameWithSuffix,"?"){
		//	if len(strings.Split(fileNameWithSuffix,"?"))>0{
		//		fileNameWithSuffix = strings.Split(fileNameWithSuffix,"?")[0]
		//	}
		//}
		fmt.Println(fileNameWithSuffix)
		//fileNameWithSuffix:=path.Base(url)
		//fileName := time.Now().Format("20060102150405")[2:]
		//fileName := strconv.Itoa(i)
		oneThreadDownloadSize := 1024 * 1024 / 2 // 一个线程下载文件的大小
		controller := DownloadThreadController{ThreadCount: thread, FileUrl: url, DownloadFolder: folder, DownloadFileName: fileNameWithSuffix}
		controller.Download(oneThreadDownloadSize)
		fmt.Println("Spend time:%v", time.Since(now))
	//}
}